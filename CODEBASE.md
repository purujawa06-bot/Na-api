...
- **Penting:** fetch polos (undici) dari Node DIBLOKIR Cloudflare 403 "Just a moment" (deteksi TLS/HTTP2 fingerprint, tak mempan walau pakai cookie+UA browser). Solusi: `lib/cloudscraper.js` (tiruan cloudscraper@4.6.0 npm berbasis fetch + tough-cookie, tanpa dep `request`; dep npm `cloudscraper` sudah dicopot karena rantai request-nya membawa vuln tanpa fix) yang mengeksekusi JS challenge CF via `node:vm` — serverless-safe.
