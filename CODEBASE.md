...
## Search Web — Yahoo + Baidu + Startpage + Wikipedia gabungan (09/2026)

`app/api/search/web/route.js` + `lib/yahoo-scrape.js` + `lib/baidu-scrape.js` + `lib/startpage-scrape.js` + `lib/wikipedia-search.js` + `lib/cookie-store.js` (file lama `lib/searxng.js`/`lib/bing-search.js` tak dipakai route, kecuali helper `suggestCorrection`).

- Tanpa pilih provider, tanpa fallback: keempat provider SELALU ditembak bersamaan via `Promise.all`; masing-masing menyumbang maks `limit` hasil (default 5 -> total maks 4x limit, cap 20) yang digabung selang-seling (1 Yahoo, 1 Baidu, 1 Startpage, 1 Wikipedia, 2 Yahoo, ...) ke SATU array (dedupe URL, `rank` ulang; tiap item ada `engine: yahoo|baidu|startpage|wikipedia`). Provider diblokir tak menggagalkan yang lain.
- Typo: bila hasil nihil/tak relevan + ada koreksi huruf ganda -> retry semuanya 1x (`corrected_from`).
- Bypass guard: cookie sesi PERSISTEN di Firebase RTDB (`yahoo_cookies` = A1/A3/dll, `baidu_cookies` = BAIDUID/dll, `startpage_cookies`; URL DB hardcode, TTL 24 jam, cache baca 10 menit) -> dipakai ulang lintas restart/proses; respons invalid -> refresh via homepage -> retry 1x + jeda sopan 1.5 detik; rotasi 3 UA desktop. **Anti-racun store (fix 09/2026): cookie hasil refresh HANYA di-persist bila retry TERBUKTI lolos** — sesi bot yang ditandai guard tidak boleh menimpa cookie bagus (dulu `refreshSession`/Baidu selalu persist walau gagal).
...
- Startpage (oportunistik): `www.startpage.com/sp/search?query=`; blok `<div class="result ...">`, judul+URL langsung dari `<a class="result-title result-link" href>` + `<h2>`, snippet `<p class="description">`. **Batasan: Anubis PoW difficulty 6 (≈16,7 jt hash ≈ 5 mnt CPU Node, melebihi maxDuration) + cookie `spchal-auth` terikat fingerprint TLS** -> dari server kemungkinan selalu `startpage_challenge` (graceful, tak menggagalkan lain).
...
- Hasil debugging CDP Brave (09/2026, IP lokal diblokir Yahoo total — koneksi di-reset bahkan untuk browser asli): cookie A1/A3/A1S browser sempat membuat fetch server lolos (guard Yahoo menilai cookie saat diterbitkan via TLS browser; cookie inti SAJA yang lolos, tambahan sBS/ymuid/lkl justru 500), tapi jatah IP habis -> sesi mati. Baidu & Wikipedia & Bing(HTML, tak dipakai) lolos dari server. Helper: `temp/cdp-fetch.cjs` (dump via CDP `127.0.0.1:9222`), `temp/seed-cookies-cdp.mjs` (seed Yahoo, hanya berguna bila IP tak diblokir).
- Uji: `node temp/test-search-web4.mjs [query]` (GET 4-provider + 400 + POST typo). Dengan Yahoo mati total, API tetap 200 via Baidu+Wikipedia.
