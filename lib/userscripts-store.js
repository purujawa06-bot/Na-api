...
(function () {
  ...
  /* ================= chess.com DOM reading ================= */
  function boardFrame() {
    return document.querySelector('.board, .board-layout, .chess-board, .board-2d');
  }
  function moveList() {
    return document.querySelector('.vertical-move-list, .horizontal-move-list, .move-list, wc-vertical-move-list, wc-horizontal-move-list, #board-layout-player-top, #board-layout-player-bottom');
  }
  function readMoves() {
    var out = [];
    var list = moveList();
    if (!list) return out;
    var nodes = list.querySelectorAll('.node');
    for (var i = 0; i < nodes.length; i++) {
      var el = nodes[i];
      if (el.classList.contains('move-index')) continue;
  ...
  function applyMovesToEngine(sanList) {
    game.reset();
    for (var i = 0; i < sanList.length; i++) {
      var u = game.moveFromSan(sanList[i]);
  ...
  function fetchMove() {
    ...
    fetch(API_URL, {
      ...
      body: JSON.stringify({ notasi: game.uci })
    }).then(function (res) {
  ...
  function refresh() {
    var sanList = readMoves();
    var key = sanList.join(',');
    if (key === lastNotasiKey) return;
    lastNotasiKey = key;
    if (!engaged || !boardFrame() || sanList.length === 0) { clearArrow(); return; }
    if (!applyMovesToEngine(sanList)) { clearArrow(); return; }
  ...
  function orientation() {
    ...
    for (var i = 0; i < pieces.length; i++) {
      ...
      var m1 = cls.match(/\s(wk|wK)\s/);
      var m2 = cls.match(/\s(bk|bK)\s/);
      if (!wk && m1) wk = pieces[i];
      if (!bk && m2) bk = pieces[i];
    }
    if (!wk || !bk || !boardFrame()) return 'w';
    return wk.getBoundingClientRect().top > bk.getBoundingClientRect().top ? 'w' : 'b';
  }
  ...
  function watchMoveList() {
    var list = moveList();
    var target = list || boardFrame() || document.body;
