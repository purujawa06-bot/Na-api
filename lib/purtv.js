...
async function getHtml(url) {
  ...
  const challenged =
    !res.ok || /<title>Just a moment/.test(html) || /challenge-error-text/.test(html);
  if (challenged) {
  ...
  if (!res.ok) throw new Error(`anichin gagal diakses (HTTP ${res.status})`);
  if (/<title>Just a moment/.test(html) || /challenge-error-text/.test(html)) {
    throw new Error('anichin menghadang Cloudflare challenge (coba lagi nanti)');
  }
...
async function moeMirrorLinks(url) {
  try {
    // Normalisasi URL episode ke host anichin.moe.
    ...
    $('.mirror option').each((_, el) => {
      ...
      try {
        ...
        // next/core-web-vitals di build Vercel. B64 hanya berisi iframe HTML.
...
export async function fetchPurtvDetail(url) {
  ...
  // Hanya server yang jelas beda dari yang sudah ada (Okru/Rumble/D-Tube/New Player).
  ...
  for (const mLink of moeLinks) {
    ...
    // Resolve Dailymotion dari mirror moe juga (kalau ada).
    let dm = null;
    try {
      if (/dailymotion\.com/i.test(mLink.url)) {
        dm = await resolveDailymotion(mLink.url);
        if (dm?.embedUrl) mLink.url = dm.embedUrl;
      }
    } catch {}
    if (dm) {
      mLink.dailymotion = dm;
      if (dm.streamUrl) mLink.streamUrl = dm.streamUrl;
      if (dm.hlsUrl) mLink.hlsUrl = dm.hlsUrl;
    }
