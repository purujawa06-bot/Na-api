...
export async function fetchPurtvDetail(url) {
  ...
  const defaultIframe = frameSrc && frameSrc !== 'about:blank'
    ...
    : null;
  // Fallback Dailymotion: video bersifat private (metadata private:true) dan
  // dibatasi domain embed — player geo bisa menolak dengan pesan
  // "tidak dapat memutar video di situs ini". Ambil id publik via metadata
  // player agar frontend bisa menampilkan tombol "Buka di Dailymotion".
