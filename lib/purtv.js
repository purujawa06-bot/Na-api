...
/**
 * Jadwal rilis — dari REST API resmi samehadaku
 * (wp-json/custom/v1/all-schedule per hari Senin–Minggu).
 * Situs anichin.com.co tidak menyediakan halaman jadwal, jadi jadwal anime
 * samehadaku dipakai agar tab Jadwal di frontend tetap terisi.
 */
export async function fetchPurtvSchedule() {
  try {
    const { fetchSamehadakuSchedule } = await samehadaku();
    const res = await fetchSamehadakuSchedule();
    ...
    return {
      source: 'purtv.vercel.app',
      schedule: [],
      note: `Jadwal rilis gagal dimuat: ${error.message}`,
      purtv_pagenation: pagenation(1, false),
    };
  }
}
