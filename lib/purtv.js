...
import {
  fetchSamehadakuHome,
  fetchSamehadakuSearchPage,
  fetchSamehadakuGenrePage,
  fetchSamehadakuDetail,
  fetchSamehadakuEpisode,
} from './samehadaku.js';
...
export async function fetchPurtvSearch(q, page = 1) {
  ...
  const [donghua, anime] = await Promise.all([
    ...
    fetchSamehadakuSearchPage(q, page).catch(() => emptyAnime()),
  ]);
...
export async function fetchPurtvList({ genre = '', page = 1 } = {}) {
  ...
  const [donghua, anime] = await Promise.all([
    ...
    genre
      ? fetchSamehadakuGenrePage(genre, page).catch(() => emptyAnime())
