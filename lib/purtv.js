 ...
 * Metadata player memetakan accessId -> id publik + judul. Dipakai untuk
 * tombol fallback "Buka di Dailymotion" saat embed dibatasi domain
 * ("tidak dapat memutar video di situs ini"). Gagal -> null.
 */
...
export async function fetchPurtvDetail(url) {
  ...
  const streamingLinks = await Promise.all(
    mirrors.map(async (m) => {
      ...
      // Samakan dgn defaultIframe: geo privat -> embed publik yg bisa diputar.
      try {
        const vm = String(iframe || '').match(/[?&]video=([^&#]+)/i);
        if (vm && vm[1]) {
          const info = await dailymotionInfo(vm[1]);
          if (info?.embedUrl) iframe = info.embedUrl;
        }
      } catch {}
  ...
  let defaultIframe = frameSrc && frameSrc !== 'about:blank'
    ...
    : null;
  // Fallback Dailymotion: video geo bersifat private (metadata private:true) dan
  // dibatasi domain embed — player geo menolak dengan pesan
  // "tidak dapat memutar video di situs ini". Ambil id publik via metadata
  // player, lalu pakai embed publik sebagai default agar langsung bisa diputar
  // cross-domain. Info dailymotion tetap dikembalikan utk tombol fallback.
  let dailymotion = null;
  try {
    const vidMatch = String(defaultIframe || '').match(/[?&]video=([^&#]+)/i);
    if (vidMatch && vidMatch[1]) dailymotion = await dailymotionInfo(vidMatch[1]);
  } catch {}
  // Pakai embed publik bila ada — ini yg bisa diputar di situs mana pun.
