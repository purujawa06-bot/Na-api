...
export async function fetchPurtvHome() {
  const [donghua, ongoingHtml, animeMod] = await Promise.all([
    getHtml(`${HOST}/`).catch(() => null),
    getHtml(`${HOST}/ongoing/`).catch(() => null),
    samehadaku(),
  ]);
  const anime = await animeMod.fetchSamehadakuHome();
  const $ = donghua ? cheerio.load(donghua) : cheerio.load('<html><body></body></html>');
  const $ongoing = ongoingHtml ? cheerio.load(ongoingHtml) : null;
  ...
  // Ongoing dengan gambar dari halaman /ongoing/ (sidebar hanya teks tanpa thumbnail)
  let ongoing = [];
  if ($ongoing) {
    ongoing = $ongoing('.listupd article.bs')
      .map((_, el) => parseCard($ongoing(el)))
      .get()
      .filter((item) => item.thumbnail);
  }
  if (ongoing.length === 0) {
    ongoing = $('.ongoingseries ul li')
      .map((_, el) => {
        const $el = $(el);
        return {
...
export async function fetchPurtvDetail(url) {
  ...
  let seriesEl = $('#breadcrumbs ol li:nth-child(3) a').first();
  if (!seriesEl.length) seriesEl = $('.ts-breadcrumb li:nth-child(2) a').first();
  ...
  return {
    ...
    // Halaman episode anichin tidak punya poster khusus — fallback berlapis:
    // og:image -> poster seri (.thumbook/.bigcontent) -> gambar pertama entry-content
    thumbnail: $('meta[property="og:image"]').first().attr('content')
      || $('.thumbook .thumb img').first().attr('src')
      || $('.bigcontent .thumb img').first().attr('src')
    ...
    synopsis:
      $('.entry-content p')
        ...
        .get()
        .filter((t) => t && !/^(download|watch)\b/i.test(t) && t.length > 40)[0] || null,
...
export async function fetchPurtvSeries(url) {
  ...
  return {
    source: 'purtv.vercel.app',
    title: $('.entry-title').first().text().trim(),
    poster: $('.thumbook .thumb img').first().attr('src') || $('.bigcontent .thumb img').first().attr('src') || $('meta[property="og:image"]').first().attr('content') || null,
    rating: $('.thumbook .rating strong').first().text().trim().replace(/^Rating\s*/i, '') || null,
