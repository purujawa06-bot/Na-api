...
// Registry model â€” choices param `model` di docs diambil dari sini, bukan
// dari JSDoc (agar cukup diubah di lib/ai-models.js).
const { ALL_MODEL_IDS } = require('./ai-models');
...
const extractExampleBodyKeys = (example) => {
    ...
    for (const b of blocks) {
        try {
            ...
            // Body berupa array langsung (mis. [\"e2e4\", ...]) -> tandai sebagai
            // direct-array agar validasi isi di-skip. Field docs hanya label UI,
            // bukan representasi objek payload.
            if (Array.isArray(obj)) {
                keys._directArray = true;
            }
...
const validateEndpointDocs = (endpoint) => {
    ...
    if (hasBody) {
        ...
        // Skip validation if the body is a direct array (no dot notation in @param body,
        // atau contoh payload berupa array langsung).
        const isDirectArrayBody = endpoint.params.some(p => p.in === 'body' && !p.name) || exampleKeys._directArray;

        if (!isDirectArrayBody) {
 ...
 * Path endpoint & URL TIDAK berubah â€” ini murni pengelompokan dokumentasi.
 */
const CATEGORY_OVERRIDES = {
    ...
    'chess/maia/route.js': 'AI',
    'text2image/route.js': 'AI',
    'play/soundcloud/route.js': 'downloader',
    'tools-image/upscaler/route.js': 'tools',
    'tools-image/remove-background/route.js': 'tools',
    'tools-image/html-to-image/route.js': 'tools',
    'dramabox/home/route.js': 'nonton/baca',
    'dramabox/category/route.js': 'nonton/baca',
    'dramabox/detail/route.js': 'nonton/baca',
    'dramabox/search/route.js': 'nonton/baca',
    'dramabox/stream/route.js': 'nonton/baca',
    'komiku/home/route.js': 'nonton/baca',
    'komiku/pustaka/route.js': 'nonton/baca',
    'komiku/detail/route.js': 'nonton/baca',
    'komiku/chapter/route.js': 'nonton/baca',
    'komiku/genre/route.js': 'nonton/baca',
    'komiku/search/route.js': 'nonton/baca',
    'purtv/home/route.js': 'nonton/baca',
    'purtv/detail/route.js': 'nonton/baca',
    'purtv/series/route.js': 'nonton/baca',
    'purtv/schedule/route.js': 'nonton/baca',
    'purtv/search/route.js': 'nonton/baca',
    'purtv/genres/route.js': 'nonton/baca',
    'purtv/list/route.js': 'nonton/baca',
};

// Kategori sistem internal yang TIDAK diekspos ke dokumentasi publik /docs.
// Endpoint admin, blog, temp, pages, dll bersifat internal dan tidak boleh
// muncul di public/docs.json.
...
const scanDocs = async () => {
    try {
        ...
        for (const file of files) {
            ...
            // Param `model` (endpoint chat completions): choices dari registry
            // lib/ai-models.js â€” JSDoc tak perlu mencantumkan @choice lagi.
            if (endpoint.path === '/api/chat/completions') {
                const modelParam = endpoint.params.find((p) => p.name === 'model');
                if (modelParam) {
                    modelParam.choices = ALL_MODEL_IDS.map((id) => ({ value: id, label: id }));
                }
            }

            // Parse @guide tag â€” custom integration guide replacing standard tabs
            const guideMatch = jsdoc.match(/@guide\s+([\s\S]*?)(?=\n\s*\*?\s*@\w|\*\/|$)/);
            if (guideMatch) {
                endpoint.guide = guideMatch[1].replace(/^\s*\*/gm, '').trim();
            }

            // Format rusak: JSDoc berniat mendokumentasikan endpoint (@title/@path/@method)
            // tapi parsing gagal (mis. tag hilang, format @param salah => field undefined).
            // Deteksi di sini, jangan biarkan lenyap diam-diam.
            if (/@(title|path|method)/.test(jsdoc)) {
                ...
                // Field wajib tidak boleh jadi `undefined`/non-string (jaga dari
                // format XML / JSDoc yang salah menghasilkan nilai tak valid).
                for (const [field, val] of Object.entries(endpoint)) {
                    if (val === undefined) {
                        docErrors.push({
                            file,
                            problems: [`field "${field}" bernilai undefined (format JSDoc/XML salah)`],
                        });
                    }
                }
                for (const p of endpoint.params || []) {
                    // name sengaja undefined utk @param body tanpa dot-notasi
                    const nameIntentionallyMissing = p.in === 'body' && !p.name;
                    for (const [field, val] of Object.entries(p)) {
                        if (val === undefined && !(nameIntentionallyMissing && field === 'name')) {
                            docErrors.push({
                                file,
                                problems: [`@param "${p.name}" field "${field}" bernilai undefined`],
                            });
                        }
                    }
                }
            ...
            if (endpoint.method && endpoint.path) {
                if (/@(title|path|method)/.test(jsdoc) && !endpoint.title) {
                    docErrors.push({ file, problems: ['tag @title tidak terbaca (format JSDoc salah)'] });
                }
                ...
                // Lewati kategori sistem internal agar tidak muncul di docs publik.
                if (INTERNAL_CATEGORIES.has(category)) continue;
        ...
        if (docErrors.length) {
            const detail = docErrors.map((e) =>
                `  âœ— ${e.file}\n${e.problems.map((p) => `      â€¢ ${p}`).join('\n')}`
            ).join('\n');
            throw new Error(
                `GENERATE DOCS GAGAL â€” dokumentasi payload tidak lengkap (${docErrors.length} file):\n${detail}\n` +
