 ...
 *
 * SANITASI URL:
 *   Gemini web otomatis membungkus URL polos jadi markdown link [url](url)
 *   (termasuk di dalam blok kode), merusak output bagi konsumen CLI. Jawaban
 *   selalu dibersihkan via unwrapMarkdownUrls(): semua [teks](url) -> teks saja.
 */
import { readFileSync } from 'node:fs';
import { fileURLToPath } from 'node:url';
import { dirname, join } from 'node:path';
import { getCookieHeader, saveCookies } from './firebase-cookie-jar.js';
...
/**
 * Bongkar semua markdown link [teks](url) -> "teks" saja (paren url dibuang).
 * Dipicu perilaku Gemini web yang membungkus URL polos jadi [url](url).
 * - Penggantian via fungsi (bukan '$1') agar label berisi $&, $1, dll. aman.
 * - Loop sampai stabil untuk nesting: [![alt](img.png)](url) -> !alt
 */
export function unwrapMarkdownUrls(text) {
  const re = /\[([^\][]*)\]\(((?:[^()]|\([^()]*\))*)\)/g;
  let out = String(text ?? '');
  for (let prev; (prev = out), true;) {
    out = out.replace(re, (_m, label) => label);
    if (out === prev) break;
  }
  return out;
}
...
async function setModel(code, cookie) {
  try {
    ...
    await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8', 'User-Agent': ua(), 'Origin': BASE, 'Referer': BASE + '/', 'Cookie': cookie }, body: body.toString() });
  } catch {}
}

/**
 * Panggil StreamGenerate (dengan cookie jar bila ada).
 * @returns {Promise<{text:string, res:Response}>} raw respons + Response (utk ambil set-cookie)
 * @throws bila HTTP bukan 2xx
 */
async function streamGenerate(inner, atToken, cookie) {
  const body = new URLSearchParams();
 ...
 * Cookie jar: baca header cookie dr RTDB -> kirim ke semua request -> setelah
 * respons, simpan Set-Cookie balik (refresh). ID jar = nama model.
 * @returns {Promise<{text:string, modelName:string}>}
 */
async function attemptGenerate(prompt, modelCode, jarModel) {
  const cookie = await getCookieHeader(jarModel);
  const { inner, at } = await buildFreq(prompt);
  await setModel(modelCode, cookie);
  const { text, res } = await streamGenerate(inner, at, cookie);
  await saveCookies(jarModel, res);
  const { modelName, answer } = parseResp(text);
  const clean = unwrapMarkdownUrls(answer);
  if (!clean || !clean.trim()) throw new Error('gemini mengembalikan konten kosong');
  return { text: clean, modelName: modelName || 'gemini' };
}

/**
 * Generate teks via Gemini web (cookies guest dipertahankan via Firebase RTDB).
 * RETRY: gagal -> coba lagi TEPAT 1x dengan sesi segar (cookie jar dibaca ulang)
 * sebelum menyerah.
 * @param {object} opts
 * @param {string} opts.prompt prompt yang dikirim
 * @param {number} [opts.modelCode] kode model L5adhe (0=Flash-Lite, 1=Flash)
 * @param {string} [opts.jarModel] ID key cookie jar di RTDB (default 'gemini-3.6-flash')
 */
export async function generateGemini({ prompt, modelCode = 1, jarModel = 'gemini-3.6-flash' }) {
  let lastErr = null;
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      return await attemptGenerate(prompt, modelCode, jarModel);
    } catch (err) {
      lastErr = err;
      if (attempt >= 2) break;
      console.error(`[gemini] percobaan 1 gagal (${err?.message ?? err}), coba lagi dengan sesi segar...`);
    }
  }
