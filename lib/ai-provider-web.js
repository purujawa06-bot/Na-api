...
import { debugEcho } from './debug-web.js';
...
// ---------------- Debug (echo + logging ke HF Space collector) ----------------

/**
 * Model echo: kirim prompt ke HF Space collector lalu echo kembali.
 * Berguna untuk pipeline testing tanpa membakar kuota provider.
 * @param {object} opts
 * @param {string} opts.modelId ID publik ('debug').
 */
export function createDebugWebModel({ modelId = 'debug' } = {}) {
  return {
    specificationVersion: 'v4',
    provider: 'debug-web',
    modelId,
    ...
    async doGenerate(options) {
      const promptText = flattenV4Prompt(options.prompt);
      const { text } = await debugEcho(promptText);
      return {
        content: [{ type: 'text', text }],
        finishReason: finish('stop'),
        usage: makeUsage(promptText, text),
...
export function createWebModel(modelId, { meta, chain } = {}) {
  ...
  if (modelId === 'debug') return createDebugWebModel({ modelId });
