 ...
 *
 * Menyatukan Gemini web (lib/gemini-web.js), Gemini 1.5 Flash via EaseMate
 * (lib/easemate-web.js), Gemini (lib/gemini-share-web.js) sebagai custom provider
 * `ai` sehingga /api/chat/completions cukup satu jalur generateText/streamText
 * untuk semua model.
 *
 * Alur:
 *   OpenAI body -> ModelMessage -> generateText/streamText
 *     -> (bila ada tools) uiTarsToolMiddleware menyuntik definisi tool sebagai teks
...
import { generateQuillbot } from './quillbot-web.js';
 ...
 *                     default gemini-3.6-flash -> gemini-1.5-flash)
 */
export { ALL_MODEL_IDS, AUTO_CHAIN_DEFAULT };

function makeUsage(inputText, outputText) {
  // Token dihitung dgn tokenizer BPE asli (js-tiktoken o200k_base) dari
  // prompt final ter-flatten (input) & teks hasil (output) — lihat lib/tokenizer.js.
  const inTok = countTokens(inputText);
  const outTok = countTokens(outputText);
  return {
...
export function flattenV4Prompt(v4Prompt = []) {
  ...
  for (const msg of v4Prompt) {
    // Middleware tool call (@ai-sdk-tool/parser, protokol UI-TARS XML) mengubah
...
/**
 * Konversi LanguageModelV4Prompt -> array messages OpenAI (native multi-role).
 * Digunakan provider yang endpoint-nya OpenAI-compatible (puru-web).
 * UI-TARS middleware sudah mengonversi tool-call/tool-result jadi teks XML,
 * jadi cukup ekstrak text dari content parts & petakan role.
 * @returns {{system?:string, messages:Array<{role,content}>}}
 */
export function toOpenAiMessages(v4Prompt = []) {
  const system = [];
  const messages = [];
  for (const msg of v4Prompt) {
    let text = '';
    if (typeof msg.content === 'string') {
      ...
      for (const p of msg.content ?? []) {
        if (p?.type === 'text' && p.text) texts.push(p.text);
        // tool-call/tool-result unexpected: UI-TARS sudah flatten ke text, tapi
        // kalau masih berupa part, fallback ke stringify agar tak hilang.
        else if (p?.type === 'reasoning' && p.text) texts.push(p.text);
        else if (p?.type === 'tool-call') texts.push(`<tool_call>${JSON.stringify({ name: p.toolName, arguments: p.input })}</tool_call>`);
        else if (p?.type === 'tool-result') {
          const val = p.output?.type === 'json' ? JSON.stringify(p.output.value) : String(p.output?.value ?? '');
          texts.push(`[hasil tool "${p.toolName}" (${p.toolCallId})]: ${val}`);
        }
      }
      text = texts.join('\n');
    }
    if (msg.role === 'system') { system.push(text); continue; }
    messages.push({ role: msg.role === 'assistant' ? 'assistant' : msg.role === 'tool' ? 'tool' : 'user', content: text || null });
  }
...
/**
 * @param {object} opts
 * @param {string} opts.modelId 'gemini-3.6-flash' (satu-satunya varian Gemini yang diekspos).
 */
export function createGeminiWebModel({ modelId = 'gemini-3.6-flash' } = {}) {
  // kode L5adhe: 0=Flash-Lite, 1=Flash (lihat lib/gemini-web.js MODELS).
  // Hanya Flash-Lite (0) yang dipakai karena varian tercepat.
  const codeByModel = { 'gemini-3.6-flash': 0 };
  return {
    ...
    async _run(options) {
      ...
      const { text, modelName } = await generateGemini({ prompt: promptText, modelCode: codeByModel[modelId] ?? 0, jarModel: 'gemini-3.6-flash' });
 ...
 * @param {object} opts
 * @param {string} opts.modelId 'gemini'.
 */
export function createGeminiShareWebModel({ modelId = 'gemini' } = {}) {
  return {
    specificationVersion: 'v4',
...
/**
 * @param {object} opts
 * @param {string} opts.modelId ID publik ('gemini-1.5-flash').
 */
export function createEaseMateWebModel({ modelId = 'gemini-1.5-flash' } = {}) {
  return {
    specificationVersion: 'v4',
...
// ---------------- Puru (Koyeb OpenAI-compatible) ----------------

/**
 * Model upstream pada Koyeb endpoint OpenAI-compatible,
 * dibungkus jadi id publik 'claude-opus-4'. Streaming real (SSE upstream dipompa
 * ke part LanguageModelV4) — pola sama dengan EaseMate.
 * Konfigurasi: PURUBOY_PURU_BASE_URL (default https://productive-alyson-nue-api-e6b8b676.koyeb.app/v1)
 * & PURUBOY_PURU_API_KEY opsional (env).
 * @param {object} opts
 * @param {string} opts.modelId ID publik ('claude-opus-4').
 */
export function createPuruWebModel({ modelId = 'claude-opus-4' } = {}) {
  return {
    specificationVersion: 'v4',
    ...
    supportedUrls: {},
    async doGenerate(options) {
      const { system, messages } = toOpenAiMessages(options.prompt);
      const promptText = flattenV4Prompt(options.prompt);
      let content = '';
      let reasoning = '';
      for await (const d of streamPuru({ system, messages })) {
        if (d.type === 'reasoning') reasoning += d.text;
        else content += d.text;
      }
    ...
    async doStream(options) {
      ...
      const iterator = streamPuru({ system, messages });
...
// ---------------- QuillBot AI chat (gpt-4.1-mini) ----------------

/**
 * Chat QuillBot via quillbot.com/api/ai-chat (cloudscraper bypass CF).
 * Buffered penuh (pola sama gemini-3.6-flash) karena transport cloudscraper.
 * @param {object} opts
 * @param {string} opts.modelId ID publik ('gpt-4.1-mini').
 */
export function createQuillbotWebModel({ modelId = 'gpt-4.1-mini' } = {}) {
  return {
    specificationVersion: 'v4',
    provider: 'quillbot-web',
    modelId,
    supportedUrls: {},
    async doGenerate(options) {
      const promptText = flattenV4Prompt(options.prompt);
      const { text } = await generateQuillbot({ prompt: promptText });
      return {
        content: [{ type: 'text', text }],
        finishReason: finish('stop'),
        usage: makeUsage(promptText, text),
...
function buildChainCandidates(chain) {
  return normalizeAutoChain(chain).map((modelId) => {
    if (modelId === 'gemini-3.6-flash') return createGeminiWebModel({ modelId: 'gemini-3.6-flash' });
    if (modelId === 'gemini') return createGeminiShareWebModel({ modelId: 'gemini' });
    if (modelId === 'claude-opus-4') return createPuruWebModel({ modelId: 'claude-opus-4' });
    if (modelId === 'gpt-4.1-mini') return createQuillbotWebModel({ modelId: 'gpt-4.1-mini' });
    return createEaseMateWebModel({ modelId: 'gemini-1.5-flash' });
  });
}
 ...
 * urutannya bisa diubah dari panel admin (default: Gemini 3.6 Flash dulu,
 ...
 * @param {string[]} [opts.chain] urutan ID provider ('gemini-3.6-flash'|'gemini-1.5-flash');
...
export function createWebModel(modelId, { meta, chain } = {}) {
  if (modelId === 'gemini-3.6-flash') return createGeminiWebModel({ modelId });
  if (modelId === 'auto') return createAutoWebModel({ meta, chain });
  if (modelId === 'claude-opus-4') return createPuruWebModel({ modelId });
  if (modelId === 'gpt-4.1-mini') return createQuillbotWebModel({ modelId });
  if (modelId === 'gemini-1.5-flash') return createEaseMateWebModel({ modelId });
  if (modelId === 'gemini') return createGeminiShareWebModel({ modelId });
