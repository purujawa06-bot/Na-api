...
/**
 * Hanya 3 ID model publik (nama sederhana, semua versi tercepat):
 *   - 'gemini-lite' : Gemini Flash-Lite via gemini.google.com
 *   - 'deepseek-v3' : DeepSeek V3 tanpa reasoning (internal: deepseek-chat)
 *   - 'auto'        : default; coba Gemini dulu, fallback DeepSeek V3 bila error/konten kosong
 */
export const ALL_MODEL_IDS = ['gemini-lite', 'deepseek-v3', 'auto'];

/** Peta internal model DeepSeek web -> flag thinking (hanya varian non-reasoning). */
...
/**
 * @param {object} opts
 * @param {string} opts.modelId ID internal DeepSeek web ('deepseek-chat').
...
// ---------------- Auto (Gemini dulu, fallback DeepSeek V3) ----------------
 ...
 *
 * @param {object} opts
 * @param {boolean} [opts.searchEnabled] diteruskan ke DeepSeek saat fallback.
 * @param {object} [opts.meta] objek output; `meta.used` diisi ID model aktual
 *                             ('gemini-lite' | 'deepseek-v3') setelah generate.
 */
export function createAutoWebModel({ searchEnabled = false, meta = {} } = {}) {
  const gemini = createGeminiWebModel({ modelId: 'gemini-lite' });
  const deepseek = createDeepSeekWebModel({ modelId: 'deepseek-chat', searchEnabled });
  ...
  function fallback(err) {
    meta.used = 'deepseek-v3';
    console.error(`[auto] Gemini gagal ("${err?.message ?? err}"), fallback ke deepseek-v3`);
    return err;
  }
  ...
  return {
    ...
    async doGenerate(options) {
      try {
        ...
        fallback(err);
        return deepseek.doGenerate(options);
      }
    },
    async doStream(options) {
      try {
        ...
        fallback(err);
        // streaming asli dari upstream DeepSeek (delta per delta)
        return deepseek.doStream(options);
      }
    },
  };
}
...
export function createWebModel(modelId, { searchEnabled = false, meta } = {}) {
  if (modelId === 'gemini-lite') return createGeminiWebModel({ modelId });
  if (modelId === 'auto') return createAutoWebModel({ searchEnabled, meta });
  if (modelId === 'deepseek-v3') return createDeepSeekWebModel({ modelId: 'deepseek-chat', searchEnabled });
