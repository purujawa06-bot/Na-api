...
export function createGeminiWebModel({ modelId = 'gemini-lite' } = {}) {
  ...
  return {
    ...
    async _run(options) {
      ...
      const { text, modelName } = await generateGemini({ prompt: promptText, modelCode: codeByModel[modelId] ?? 0, jarModel: 'gemini-lite' });
 ...
 * Buffered penuh (pola sama gemini-lite) karena transport cloudscraper.
 * @param {object} opts
 * @param {string} opts.modelId ID publik ('quillbot').
 */
export function createQuillbotWebModel({ modelId = 'quillbot' } = {}) {
  return {
    specificationVersion: 'v4',
...
function buildChainCandidates(chain) {
  return normalizeAutoChain(chain).map((modelId) => {
    ...
    if (modelId === 'quillbot') return createQuillbotWebModel({ modelId: 'quillbot' });
...
export function createWebModel(modelId, { meta, chain } = {}) {
  ...
  if (modelId === 'quillbot') return createQuillbotWebModel({ modelId });
