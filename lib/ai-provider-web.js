 ...
 *   - 'easemate'    : EaseMate AI via api.easemate.ai (sign WASM, kuota per-IP)
 *   - 'auto'        : default; menyusuri rantai fallback (error/konten kosong
 *                     -> provider berikutnya; urutan diatur admin via settings,
 *                     default gemini-lite -> deepseek-v4 -> easemate)
 */
export const ALL_MODEL_IDS = ['gemini-lite', 'deepseek-v4', 'easemate', 'auto'];
...
/** Stream palsu satu-chunk dari teks yang sudah utuh (dipakai model Gemini). */
...
// ------- Auto (rantai fallback dinamis; default Gemini -> DeepSeek V4 -> EaseMate) -------

/**
 * Urutan DEFAULT rantai mode 'auto'. Urutan aktual bisa diubah admin lewat
 * panel (public/admin.html tab "Auto Model" -> POST /api/admin/aichain ->
 * tabel settings DB), dibaca ulang route tiap request (cache 60s).
 */
export const AUTO_CHAIN_DEFAULT = ['gemini-lite', 'deepseek-v4', 'easemate'];
const AUTO_CHAIN_ALLOWED = AUTO_CHAIN_DEFAULT;
...
function buildChainCandidates(chain) {
  return normalizeAutoChain(chain).map((modelId) => {
    ...
    if (modelId === 'easemate') return createEaseMateWebModel({ modelId: 'easemate' });
    return createNoteGPTWebModel({ modelId: 'deepseek-v4' });
  });
}
 ...
 * fallback DeepSeek V4 via NoteGPT).
 *
 ...
 * @param {string[]} [opts.chain] urutan ID provider ('gemini-lite'|'deepseek-v4'|'easemate');
