 ...
 * — gabungan string pada indeks [1]. Bila gagal -> retry 1x (cookies guest
 * dipertahankan lewat Firebase RTDB; snap cookie & simpan kembali tiap request).
 */
...
async function attemptFetch(shareId, message = '', cookie) {
...
export async function generateGeminiShare(message = '', jarModel = 'gemini') {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      return { text: await attemptFetch(shareId, message, cookie), modelName: 'gemini' };
    } catch (err) {
      lastErr = err;
      if (attempt >= 2) break;
      console.error(`[gemini] percobaan 1 gagal (${err?.message ?? err}), coba lagi...`);
    }
  }
