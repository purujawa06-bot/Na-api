...
async function attemptFetch(shareId, message = '', cookie) {
  ...
  await saveCookies('gemini-share', res);
...
export async function generateGeminiShare(message = '', jarModel = 'gemini-share') {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      return { text: await attemptFetch(shareId, message, cookie), modelName: 'gemini-share' };
    } catch (err) {
      lastErr = err;
      if (attempt >= 2) break;
