 ...
 * menjadi format XML murni (morph) sebelum sampai ke morphXmlToolMiddleware.
 *
 ...
 *   - "morph" (default): <x><p>v</p>...</x>
 *   - "qwen":           <tool_call><function=x><parameter=p>v</parameter>...</function></tool_call>
 */
 ...
 * @returns {string}
 */
export function convertDsml(region, target = 'morph') {
 ...
 */
function createDsmlMachine(target = 'morph') {
 ...
 */
export function dsmlPipe(stream, target = 'morph') {
 ...
 * @returns {string}
 */
function sanitizeFull(text, target = 'morph') {
  let out = '';
  let i = 0;
  for (;;) {
...
/**
 * Middleware sanitizer dengan target protokol output ("morph" default | "qwen").
 ...
 */
export function createDsmlSanitizerMiddleware({ target = 'morph' } = {}) {
  return {
