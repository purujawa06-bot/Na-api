 ...
 *
 * Latar belakang: model web (gemini-3.6-flash/gemini-1.5-flash via mode auto)
 ...
 *   </ElicitationsGroup>
 *
 * Varian kedua: blok fence komponen UI dari provider web,
 *
 *   ```json?chameleon
 *   { "component": "Bento", ... }
 *   ```
 *
 * dibuang utuh sampai fence tutup (atau sampai habis bila tak tertutup).
 * Fence kode biasa (```js ...) tidak disentuh.
 *
 * Sanitizer pasif: teks tanpa token artefak lolos utuh, aman dipasang
...
const ORPHAN_TAG_RE = /<(?:\/?Elicitation\b|\/?FollowUp\b)[^>]*>/g;
// Blok fence chameleon: baris ```...?chameleon ... sampai fence tutup (atau EOF).
const CHAM_FULL_RE = /```[^\n]*\?chameleon[^\n]*\n[\s\S]*?(?:```|$)/g;

/** Deteksi cepat apakah teks memuat token artefak (untuk skip kerja bila bersih). */
const HINT_RE = /Elicitation|FollowUp|chameleon/i;

/**
 * Sanitasi teks utuh (jalur generate): buang blok chameleon, grup utuh,
 * grup tak tertutup, lalu tag yatim. Prose lain lolos apa adanya.
 * @param {string} text
 * @returns {string}
 */
export function sanitizeUiArtifacts(text) {
  return text
    .replace(CHAM_FULL_RE, '')
...
const TOKENS = ['<ElicitationsGroup', '<Elicitation', '</ElicitationsGroup>', '</Elicitation>', '<FollowUp', '</FollowUp>'];
// Token pembuka nama lengkap + karakter pembatas (spasi, tutup, slash).
const TOKEN_NAMED_RE = /^<(\/?)(?:Elicitation(?:sGroup)?|FollowUp)(?=[\s/>])/;
...
// --- chameleon fence artefak (provider web) ---
const CHAM_OPEN_RE = /^```[^\n]*\?chameleon[^\n]*$/;
const CHAM_CLOSE_RE = /^```\s*$/;

/**
 * State machine streaming: buang blok fence ```json?chameleon … ``` utuh.
 * Fence tutup harus baris sendiri (```\r?\n). Fence biasa lolos.
 * Flush mempertahankan kandidat prefix tak selesai — kemungkinan besar
 * kode pengguna, bukan artefak (fail-open).
 */
function createChameleonMachine() {
  let pending = '';
  ...
  function feedIdle(text, emit) {
    ...
    for (;;) {
      if (atLineStart) {
        if (!pending) return;
        if (pending.startsWith('```') || /^`{1,2}$/.test(pending)) {
          const nl = pending.indexOf('\n');
          if (nl < 0) {
            if (pending.length > MAX_TAG_HOLD) { emit(pending); pending = ''; atLineStart = false; }
            return;
          }
          const line = pending.slice(0, nl);
          pending = pending.slice(nl + 1);
          atLineStart = false;
          if (CHAM_OPEN_RE.test(line)) {
            inBlock = true; buf = '';
            const rest = pending; pending = '';
            if (feedBlock(rest, emit)) continue;
            return;
          }
          emit(line + '\n');
          continue;
      ...
      const nl = pending.indexOf('\n');
      if (nl < 0) { if (pending) { emit(pending); pending = ''; } return; }
  ...
  /** Return true bila blok ketutup & kembali idle. */
  function feedBlock(text, emit) {
    buf += text;
    for (;;) {
      const nl = buf.indexOf('\n');
      if (nl < 0) {
        // ponytail: tahan ekor ≤64 utk close terbelah; lebih dari itu buang.
        if (buf.length > 64) buf = buf.slice(-64);
        return false;
      }
      const line = buf.slice(0, nl);
      buf = buf.slice(nl + 1);
      if (CHAM_CLOSE_RE.test(line)) {
        inBlock = false;
        const rest = buf; buf = '';
        pending = rest; atLineStart = true;
        return true;
      }
    }
  }
  ...
  return {
    ...
    flush(emit) {
      if (inBlock) { inBlock = false; buf = ''; pending = ''; return; }
      // Kandidat tak selesai (mis. ```js tanpa newline) kemungkinan kode
      // pengguna → loloskan (fail-open).
      if (pending) { emit(pending); pending = ''; }
    },
  };
}

/** Ubah machine menjadi TransformStream yang sesuai protokol AI SDK stream. */
function transformFor(machine) {
  ...
  return new TransformStream({
    ...
    flush(ctl) { machine.flush((t) => emit(t, ctl)); },
  });
}
...
export function uiArtifactPipe(stream) {
  return stream
    .pipeThrough(transformFor(createChameleonMachine()))
    .pipeThrough(transformFor(createUiArtifactMachine()));
}
