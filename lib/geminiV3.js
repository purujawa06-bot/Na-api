 ...
 */
async function chatV2(promptText) {
 ...
 */
async function chatV1(promptText) {
    ...
    const conversation_id = savedData.conversation_id || '';
    const response_id = savedData.response_id || '';
    const choice_id = savedData.choice_id || '';

    if (!cookie || !snlm0e || !fsid) {
        throw new Error('Gemini V1: Cookie, SNlM0e, atau FSID tidak lengkap');
    }
    ...
    // Parse response
    const parsed = parseV1Response(response.data);
...
async function chat(promptText) {
    if (!promptText || typeof promptText !== 'string') {
        throw new Error("Parameter 'prompt' harus berupa string.");
    }

    const attemptConfigs = [
        { method: 'v2', label: 'V2 (BatchExecute)' },
        { method: 'v1', label: 'V1 (StreamGenerate)' },
        { method: 'v2', label: 'V2 (BatchExecute) — Retry' }
    ];
    ...
    for (let i = 0; i < attemptConfigs.length; i++) {
        ...
        try {
            const result = attempt.method === 'v2'
                ? await chatV2(promptText)
                : await chatV1(promptText);
