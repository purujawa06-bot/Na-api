 ...
 */
async function chatV1(promptText, retryCount = 0) {
    ...
    // Jika retry, gunakan empty IDs untuk start new conversation
    const conversation_id = retryCount > 0 ? '' : (savedData.conversation_id || '');
    const response_id = retryCount > 0 ? '' : (savedData.response_id || '');
    const choice_id = retryCount > 0 ? '' : (savedData.choice_id || '');
