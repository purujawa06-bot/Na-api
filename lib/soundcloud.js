...
/* ------------------------------------------------------------------ */
/* Pencarian (hasil reverse-engineering, Agustus 2026)                  */
/*                                                                      */
/* GET api-v2.soundcloud.com/search?q=..&limit=..&offset=..&client_id=.. */
/*   - Tanpa param lain pun sudah 200 dari server-side (tidak perlu      */
/*     user_id / facet / app_version seperti versi browser).             */
/*   - linked_partitioning=1 membuat response berisi next_href, tapi     */
/*     paginasi cukup pakai offset biasa.                                */
/*   - Path khusus per tipe: /search/tracks, /search/playlists,          */
/*     /search/albums, /search/users. Tanpa path tipe = campuran.        */
/*   - Item collection bisa track lengkap atau stub {id,kind,policy}     */
/*     (browser mengisinya via GET /tracks?ids=..; di sini stub          */
/*     langsung dibuang agar output konsisten).                          */
/* ------------------------------------------------------------------ */

/** Rapikan satu item hasil search sesuai kind-nya (track/playlist/user). */
function shapeSearchItem(x) {
  if (x.kind === 'track') {
    return {
      type: 'track',
      id: String(x.id),
      ...
      duration_ms: x.full_duration || x.duration,
      artwork_url: x.artwork_url?.replace('-large.jpg', '-t500x500.jpg') || null,
      genre: x.genre || null,
      playback_count: x.playback_count ?? null,
      likes_count: x.likes_count ?? null,
      policy: x.policy || null,
      streamable: !!x.streamable,
      is_preview: x.policy === 'SNIP',
      author: x.user ? { username: x.user.username, permalink_url: x.user.permalink_url } : null,
    };
  }
  if (x.kind === 'playlist') {
    const isAlbum = x.is_album || x.set_type === 'album';
    return {
      ...
      artwork_url: x.artwork_url?.replace('-large.jpg', '-t500x500.jpg') || null,
      track_count: x.track_count ?? null,
      likes_count: x.likes_count ?? null,
      author: x.user ? { username: x.user.username, permalink_url: x.user.permalink_url } : null,
    };
  }
  if (x.kind === 'user') {
    return {
      ...
      username: x.username,
      permalink_url: x.permalink_url,
      avatar_url: x.avatar_url?.replace('-large.jpg', '-t500x500.jpg') || null,
      followers_count: x.followers_count ?? null,
      track_count: x.track_count ?? null,
      verified: !!x.verified,
      city: x.city || null,
    };
...
/**
 * Pencarian SoundCloud (track / playlist / album / user).
 * @param {string} query - kata kunci pencarian
 * @param {object} [opts]
 * @param {'all'|'tracks'|'playlists'|'albums'|'users'} [opts.type='all'] - filter jenis hasil
 * @param {number} [opts.limit=20] - jumlah hasil per halaman (maks 50)
 * @param {number} [opts.offset=0] - offset paginasi
 * @returns {Promise<object>} { total_results, results[], next_offset|null }
 */
export async function searchSoundCloud(query, opts = {}) {
  const type = ['all', 'tracks', 'playlists', 'albums', 'users'].includes(opts.type)
    ? opts.type
    : 'all';
  ...
  const clientId = await getClientId();
  const path = type === 'all' ? '/search' : `/search/${type}`;
  const data = await api(path, { q: query, limit, offset }, clientId);
  ...
  if (data.next_href) {
    try {
      nextOffset = new URL(data.next_href, API).searchParams.get('offset');
      nextOffset = nextOffset != null ? Number(nextOffset) : offset + results.length;
    } catch {
      nextOffset = offset + results.length;
    }
  }

  return { total_results: data.total_results ?? null, results, next_offset: nextOffset };
}
