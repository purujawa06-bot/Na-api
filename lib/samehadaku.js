...
async function getHtml(url) {
    ...
    if (!res.ok) throw new Error(`samehadaku gagal diakses (HTTP ${res.status})`);
    return res.text();
}
...
export async function fetchSamehadakuSearch(q) {
    ...
    try {
        ...
        if (!res.ok) throw new Error(`samehadaku search gagal (HTTP ${res.status})`);
...
export async function fetchSamehadakuPlayer(post, nume, type) {
    ...
    try {
        ...
        if (!res.ok) throw new Error(`samehadaku player gagal (HTTP ${res.status})`);
...
export async function fetchSamehadakuJadwal(day) {
    ...
    try {
        ...
        if (!res.ok) throw new Error(`samehadaku jadwal gagal (HTTP ${res.status})`);
        const json = await res.json();
