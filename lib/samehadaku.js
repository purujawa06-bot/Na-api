 ...
 * tanpa eksekusi challenge — lebih robust dari cloudscraper. Import dilakukan
 * lazy (dinamis) supaya saat collect-page-data build tetap aman (browserslist
 * memakai require dinamis yang bisa merusak analisis webpack).
 */
...
function gotScraping() {
  if (!gotScrapingPromise) gotScrapingPromise = import('got-scraping').then((m) => m.gotScraping);
...
function parsePostShowItem($, li) {
    ...
    const epAuthor = $(li).find('.dtla span author').first().text().trim();
    const epM = epAuthor.match(/^(\d+)/);
    const author = $(li).find('.dtla span[itemprop="author"] author').text().trim() || null;
    ...
    return {
        ...
        episode: epM ? epM[1] : (epAuthor || null),
        author,
        released,
    };
}
...
/** parser kartu gaya PurTV (selector dari api.js) — dipakai saat gabung dgn anichin */
function parsePurtvCard($, article) {
    const a = $(article).find('a').first();
    const href = a.attr('href') || null;
    return {
        title: $(article).find('.data .title h2').text().trim() || null,
        thumbnail: absUrl($(article).find('.content-thumb img').first().attr('src') || null),
        url: absUrl(href),
        slug: slugOf(href),
        type: $(article).find('.content-thumb .type').text().trim() || null,
        status: $(article).find('.data .type').text().trim() || null,
        episode: $(article).find('.score').text().trim() || null,
    };
}
...
export async function fetchSamehadakuHome() {
    try {
        ...
        const items = [];
        $('.post-show ul li').each((_, li) => {
            items.push(parsePostShowItem($, $(li)));
        });
        const popularAnime = [];
        $('.topten-animesu ul li').each((_, li) => {
            const $li = $(li);
            popularAnime.push({
                title: $li.find('.judul').text().trim() || null,
                thumbnail: absUrl($li.find('img').first().attr('src') || null),
                url: absUrl($li.find('a').first().attr('href')),
                rating: $li.find('.rating').text().trim() || null,
                rank: $li.find('.is-topten').text().trim() || null,
            });
        });
        const pagination = parsePagination($);
        return { source: 'v2.samehadaku.how', homepage: items, popularAnime, pagination };
    } catch (err) {
...
/**
 * @summary Cari anime di samehadaku (HTML, pola PurTV dgn paginasi)
 * @param {string} q - Kata kunci pencarian
...
export async function fetchSamehadakuSearchPage(q, page = 1) {
    ...
    try {
        ...
        $('article.animpost').each((_, article) => {
            results.push(parsePurtvCard($, $(article)));
        });
        const hasNext = $('.pagination .next').length > 0;
        return { source: 'v2.samehadaku.how', query: q, page, results, hasNext };
    } catch (err) {
        return { source: 'v2.samehadaku.how', error: err.message };
    }
}
...
/**
 * @summary Ambil daftar anime per genre (HTML, pola PurTV dgn paginasi)
...
export async function fetchSamehadakuGenrePage(slug, page = 1) {
    ...
    try {
        ...
        const hasNext = $('.pagination .arrow_pag').length > 0;
        return { source: 'v2.samehadaku.how', genre: slug, page, results, hasNext };
    } catch (err) {
        return { source: 'v2.samehadaku.how', error: err.message };
    }
}
