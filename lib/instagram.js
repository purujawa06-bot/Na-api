 ...
 *      CDN Instagram (*.cdninstagram.com atau *.fbcdn.net, mp4/jpg).
 *      Invalid shortcode di-redirect balik ke instagram.com;
 *      shortcode tak dikenal -> HTTP 504.
...
async function viaKkInstagram(shortcode) {
  ...
  for (const path of paths) {
    ...
    // Host berubah-ubah: scontent.cdninstagram.com (lama) atau
    // instagram.*.fbcdn.net (per 09/2026) — terima keduanya.
    if (res.status >= 300 && res.status < 400 && loc && /^https?:\/\/[^/]*(?:cdninstagram\.com|fbcdn\.net)/i.test(loc)) {
