...
async function fetchAtToken(client) {
    // Daftar URL yang akan dicoba untuk extract token
    const urls = [
        'https://gemini.google.com/',
        'https://gemini.google.com/app',
        'https://gemini.google.com/u/0/'  // Alternate path
    ];
    ...
    // Jika semua URL gagal, coba dengan request ke batchexecute sebagai fallback
    // (beberapa versi Gemini mengembalikan token di response header)
    try {
        ...
        const setCookie = probeResp.headers['set-cookie'];
        if (setCookie) {
            // Beberapa token disimpan di cookie
        }
    } catch (err) {
        lastError = err;
    }
    
    throw new Error(`Gagal fetch at token dari semua sumber: ${lastError?.message || 'tidak ada response'}`);
}
...
async function chat(promptText, retryCount = 0) {
    ...
    try {
        ...
        const isTokenError = err.message?.includes('at token') || err.message?.includes('SNlM0e');
        
        if ((isInvalidArg || is400 || isTokenError) && retryCount < 1) {
            // Force fresh token fetch on retry — hapus token lama dari Firebase
            await syncFirebase('PUT', {
                cookies: null,
                at_token: null
            });
