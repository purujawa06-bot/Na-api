/**
 * Satu langkah agent (generateText + tools) — dipakai oleh
 * /api/chat/completions (mode non-streaming).
 *
...
import { hermesToolMiddleware, morphXmlToolMiddleware } from '@ai-sdk-tool/parser';
import { jsonSchema } from '@ai-sdk/provider-utils';
import { createWebModel } from './ai-provider-web.js';
import settingsService from './settingsService';

/** Pilih middleware tool-calling protocol. */
function toolMiddleware(protocol = 'hermes') {
  return protocol === 'morphxml' ? morphXmlToolMiddleware : hermesToolMiddleware;
}
...
/** Susun model siap generate: adapter web + middleware sanitizer + tools protocol. */
export function buildModel(modelId, { tools, meta, chain, protocol = 'hermes' }) {
  const base = createWebModel(modelId, { meta, chain });
  if (!tools || !Object.keys(tools).length) return base;
  return wrapLanguageModel({
    model: base,
    middleware: toolMiddleware(protocol),
  });
}
