...
export async function uploadFile(token, buffer, filename = 'image.png', contentType = 'image/png') {
  // Server hanya menerima ±2 pemakaian per header PoW -> bila dibalas 40301
  // INVALID_POW_RESPONSE, buang cache & solve ulang SEKALI (pola sama dgn streamTurn).
  for (let attempt = 1; attempt <= 2; attempt++) {
    const pow = await getPowHeader(token, POW_UPLOAD);
    const fd = new FormData();
    fd.append('file', new Blob([buffer], { type: contentType }), filename);
    // ponytail: header ini wajib agar backend menandai file sbg model_kind VISION
    // (tanpa-nya jadi NORMAL -> vision tak bisa parse). x-file-size & model-type
    // vision dibutuhkan di sini (dgn x-ds-pow-response dari getPowHeader).
    const h = {
      ...baseHeaders(token, false),
    ...
    if (res.data?.biz_code === 40301 && attempt < 2) {
      powCache.delete(`${token}:${POW_UPLOAD}`);
      continue;
    }
  ...
  throw new Error('upload file gagal: PoW retry habis');
}
 ...
 * @param {string} [opts.modelType] model_type completion; default 'vision' bila ada refFileIds (default menolak file vision)
 * @param {boolean} [opts.searchEnabled] search_enabled (jaringan web) dikirim ke DeepSeek
 ...
 */
export async function* streamTurn({ token, chatId, parentId = null, prompt, thinkingEnabled = false, refFileIds = [], modelType, searchEnabled = false, meta = {} }) {
  if (modelType === undefined) modelType = refFileIds.length ? 'vision' : 'default';
 ...
 */
export async function askDeepSeek({ token, prompt, chatId, parentId, thinking, imageUrl, stream, modelType, searchEnabled }) {
  ...
  (async () => {
    ...
    for await (const part of streamTurn({ token, chatId: sessionId, parentId: parentId ?? null, prompt, thinkingEnabled, refFileIds, modelType, searchEnabled, meta })) {
