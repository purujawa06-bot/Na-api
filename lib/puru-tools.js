 ...
 *
 * Dipakai oleh:
 *  - /api/tools/execute  (eksekusi tool generik: search, crawl, docs, fetch)
 *
 * Tool yang tersedia:
  ...
  return {
    tools: [
      ...
      },
      {
        ...
        function: {
          name: 'fetch',
          description: 'Fetch data dari URL menggunakan HTTP. Parameter "url" WAJIB diisi (string). Contoh: fetch({ url: "https://example.com" }) atau fetch({ url: "/api/search/duckduckgo?query=react" }). Berguna untuk testing API endpoint atau mengambil data dari URL tertentu.',
          parameters: {
            type: 'object',
            properties: {
