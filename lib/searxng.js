 ...
 *   - Blacklist persisten di Firebase RTDB (URL hardcode): instance
 *     yang error/mati/non-JSON/lemot (>5 detik) dicatat + difilter
 *     dari kandidat sehingga digantikan kandidat sehat lain (TTL 24 jam).
...
/** Firebase Realtime DB untuk blacklist instance (URL di-hardcode sesuai permintaan). */
const FIREBASE_DB_URL = 'https://puru-69425-default-rtdb.firebaseio.com/';
const BLACKLIST_PATH = 'searxng_blacklist';

/** TTL blacklist 24 jam (instance bisa pulih) + cache baca 10 menit. */
const BLACKLIST_TTL_MS = 24 * 60 * 60 * 1000;
const BLACKLIST_CACHE_MS = 10 * 60 * 1000;

let blacklistCache = { at: 0, map: {} };

/** Key RTDB aman (tanpa . $ # [ ] /) dari base URL instance. */
function blacklistKey(base) {
  return String(base)
    .replace(/^https?:\/\//, '')
    .replace(/\/+$/, '')
...
/**
 * Baca blacklist dari Firebase (fail-open: gagal -> anggap kosong).
 * Entri kedaluwarsa (>24 jam) diabaikan agar instance pulih otomatis.
 */
async function getBlacklist() {
  if (Date.now() - blacklistCache.at < BLACKLIST_CACHE_MS) return blacklistCache.map;
  try {
    const res = await fetch(`${FIREBASE_DB_URL}${BLACKLIST_PATH}.json`, {
    ...
    if (body && typeof body === 'object') {
      for (const [k, v] of Object.entries(body)) {
        if (v && Date.now() - (v.at || 0) < BLACKLIST_TTL_MS) map[k] = v;
      }
    }
    blacklistCache = { at: Date.now(), map };
  } catch {
    /* abaikan -> pakai cache lama / kosong */
  }
  return blacklistCache.map;
}

/**
 * Masukkan instance ke blacklist Firebase: error, mati, atau > 5 detik.
 * Cache lokal di-update langsung; gagal tulis diabaikan (tak boleh
 * menggagalkan search).
 */
async function blacklistInstance(base, reason) {
  const key = blacklistKey(base);
  blacklistCache.map[key] = { at: Date.now(), reason };
  try {
    await fetch(`${FIREBASE_DB_URL}${BLACKLIST_PATH}/${key}.json`, {
      method: 'PUT',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ at: Date.now(), reason }),
 ...
 * Tanpa hardcode: murni hasil probe dinamis. Kandidat yang masuk
 * blacklist Firebase dilewati agar otomatis digantikan kandidat sehat lain.
 */
async function getWorkingInstances(force = false) {
  ...
  refreshPromise = (async () => {
    try {
      const [candidates, bl] = await Promise.all([getCandidates(), getBlacklist()]);
      const fresh = candidates.filter((base) => !bl[blacklistKey(base)]);
      const probed = (
        await Promise.all(fresh.map((base) => probeInstance(base)))
      ).filter(Boolean);
 ...
 * Instance yang error/mati/lemot (>5 detik) otomatis masuk blacklist
 * Firebase agar digantikan kandidat sehat lain.
 */
async function fetchInstance(base, query, lang) {
  ...
  try {
    ...
    if (!res.ok) {
      await blacklistInstance(base, `http_${res.status}`);
      return null;
    }
    let body;
    try {
      ...
      await blacklistInstance(base, 'non_json');
      return null;
    }
    if (!body || !Array.isArray(body.results)) {
      await blacklistInstance(base, 'bad_shape');
      return null;
    }
    if (Date.now() - started > REQUEST_TIMEOUT_MS) {
      await blacklistInstance(base, `slow_${((Date.now() - started) / 1000).toFixed(1)}s`);
    }
    return {
      base,
    ...
    const reason = /abort|timeout/i.test(e?.cause?.message || e?.message || '') ? 'timeout_5s' : 'error';
    await blacklistInstance(base, reason);
    return null;
  }
}
