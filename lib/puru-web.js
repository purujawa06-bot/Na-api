 ...
 * Upstream: HF Space https://nirkyy-a.hf.space/v1 dengan model 'puru'.
 * Endpoint ini adalah proxy chat completions polos,
 * jadi cukup satu jalur fetch untuk streaming (SSE) maupun non-streaming.
 *
 * Konfigurasi lewat env (local & Vercel):
 *   PURUBOY_PURU_BASE_URL  default https://nirkyy-a.hf.space/v1
 *   PURUBOY_PURU_API_KEY   dikirim sebagai Bearer (wajib; ada default fallback)
 */

const DEFAULT_BASE_URL = 'https://nirkyy-a.hf.space/v1';
const DEFAULT_API_KEY = 'sk-843e3f05f05eacfe-55n2je-f2c2b844';

/** Model upstream yang diminta (dibungkus sebagai 'puru' di publik). */
export const PURU_UPSTREAM_MODEL = 'puru';
...
function getApiKey() {
  return process.env.PURUBOY_PURU_API_KEY || DEFAULT_API_KEY;
}
 ...
 * @param {object} opts
 * @param {Array<{role:string,content:string|null}>} opts.messages array pesan OpenAI multi-role
 * @param {string} [opts.system] system prompt opsional (didahulukan sebagai role system)
 * @param {string} [opts.model] ID upstream (default 'puru')
 * @yields {{type:'text'|'reasoning', text:string}}
 */
export async function* streamPuru({ messages = [], system, model = PURU_UPSTREAM_MODEL } = {}) {
