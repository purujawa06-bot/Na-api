...
const isSeriesUrl = (u = '') => /\/seri\/|\/series\/|\/anime\//.test(u || '');
...
export default function PurTVPage() {
  ...
  const openSch = async () => { setShowSch(true); setDay(0); if (!schedule) { try { const s = await fetchJson(API + '/schedule'); setSchedule(s); } catch { setSchedule({ schedule: [], donghuaSchedule: [] }); } } };
  const onInput = (v) => {
    setInput(v);
  ...
  const animeDays = asArray(schedule?.schedule);
  const donghuaDays = asArray(schedule?.donghuaSchedule);
  const donghuaFlat = asArray(schedule?.donghua);
  const days = animeDays.length ? animeDays : donghuaDays.length ? donghuaDays : [];
  const curD = days[day] || days[0];
  const schNote = asString(schedule?.note);
  const hasDonghuaSch = donghuaDays.length > 0 || donghuaFlat.length > 0;
...
.skel{background:linear-gradient(90deg,#161922,#1f2535,#161922);background-size:200% 100%;animation:sh 1.2s infinite;border-radius:.8rem}@keyframes sh{to{background-position:-200% 0}}
      ...
      {showSch && (<><div className="side-overlay" onClick={() => setShowSch(false)} /><div className={'side-panel' + (showSch ? ' open' : '')}><div className="p-4 border-b border-white/5 flex items-center gap-3"><span className="w-9 h-9 rounded-xl flex items-center justify-center text-white" style={{ background: 'linear-gradient(135deg,#3b82f6,#2563eb)' }}><i className="fas fa-calendar-day text-sm" /></span><div className="flex-1"><h3 className="text-sm font-black text-white">Jadwal Rilis</h3><p className="text-[10px] text-gray-500">Anime per hari + Donghua terbaru</p></div><button onClick={() => setShowSch(false)} className="icon-btn2"><i className="fas fa-times" /></button></div><div className="p-3 flex gap-2 overflow-x-auto border-b border-white/5">{animeDays.map((dd, i) => <button key={i} onClick={() => setDay(i)} className={'day-chip' + (day === i ? ' on' : '')}>{asString(dd.day || 'Hari ' + (i + 1)).slice(0, 10)}</button>)}{!animeDays.length && !donghuaDays.length && <span className="text-[11px] text-gray-500">Memuat...</span>}{!animeDays.length && donghuaDays.length > 0 && <span className="text-[11px] text-gray-500">Jadwal anime kosong — tampilkan donghua di bawah</span>}</div><div className="flex-1 overflow-y-auto p-3 space-y-2">{animeDays.length > 0 ? asArray(curD?.list).map((it, i) => <button key={i} onClick={() => { setShowSch(false); clickCard(it); }} className="hist-row w-full text-left"><span className="relative w-11 h-16 rounded-lg overflow-hidden bg-black flex-shrink-0">{it.thumbnail && <Image src={it.thumbnail} alt="" fill sizes="44px" className="object-cover" unoptimized />}</span><span className="flex-1 min-w-0"><span className="block text-[11px] font-bold text-white truncate">{asString(it.title)}</span><span className="block text-[10px] text-blue-400 font-bold mt-0.5">{asString(it.releaseTime || it.nextEpisode || it.episode || '')}</span></span></button>) : null}{animeDays.length > 0 && !asArray(curD?.list).length && <div className="panel p-6 text-center"><p className="text-[11px] text-gray-400 font-bold">Tidak ada jadwal untuk {asString(curD?.day || 'hari ini')}.</p></div>}<div className="pt-1"><h4 className="mini-head"><i className="fas fa-fire mr-2 text-rose-400" />Donghua Terbaru</h4>{asArray(donghuaDays[0]?.list?.length ? donghuaDays[0].list : donghuaFlat).slice(0, 20).map((it, i) => <button key={'d' + i} onClick={() => { setShowSch(false); clickCard(it); }} className="hist-row w-full text-left mb-2"><span className="relative w-11 h-16 rounded-lg overflow-hidden bg-black flex-shrink-0">{it.thumbnail && <Image src={it.thumbnail} alt="" fill sizes="44px" className="object-cover" unoptimized />}</span><span className="flex-1 min-w-0"><span className="block text-[11px] font-bold text-white truncate">{asString(it.title)}</span><span className="block text-[10px] text-rose-400 font-bold mt-0.5">{asString(it.episode || it.releaseTime || it.type || 'Update')}</span></span></button>)}{!hasDonghuaSch && <div className="panel p-5 text-center"><p className="text-[11px] text-gray-400 font-bold">Respon dari anichin tidak ada.</p><p className="text-[10px] text-gray-500 mt-1">Anichin kena Cloudflare — coba lagi nanti.</p></div>}{schNote && <p className="text-[9px] text-gray-600 mt-2">{schNote}</p>}</div></div></div></>)}
