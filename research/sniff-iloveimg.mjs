#!/usr/bin/env node
/**
 * CDP Sniffer untuk iloveimg.com/upscale-image
 *
 * Usage:
 *   node research/sniff-iloveimg.mjs            # sniff 60 detik default
 *   node research/sniff-iloveimg.mjs --ms 30000
 */
...
const TARGET = 'https://www.iloveimg.com/upscale-image';
const OUT = join(dirname(fileURLToPath(import.meta.url)), '.iloveimg-sniff.json');
const args = process.argv.slice(2);
const msIdx = args.indexOf('--ms');
const DURATION_MS = msIdx !== -1 ? parseInt(args[msIdx + 1], 10) : 60000;
...
async function connect() {
  ...
  let page = targets.find(
    (t) => t.type === 'page' && t.url.includes('iloveimg.com')
  );
  if (!page) {
    page = await (
...
const STATIC_EXT = /\.(png|jpe?g|webp|gif|ico|woff2?|ttf|eot|svg|css|js|map)(\?|$)/i;
...
function isApi(url) {
  try {
    ...
    if (u.hostname.endsWith('iloveimg.com')) return true;
    if (/ilovepdf|iloveimg|api|upload|worker|storage|cdn/i.test(u.hostname)) return true;
    return false;
  } catch { return false; }
}
...
onEvent(async (msg) => {
  if (msg.method === 'Network.requestWillBeSent') {
    ...
    seenRequests.set(msg.params.requestId, {
      ...
      postData: r.postData ? r.postData.slice(0, 50000) : null,
      ts: Date.now(),
    });
  }
...
async function main() {
  ...
  if (!page.url.includes('iloveimg.com/upscale-image')) {
    console.log('[i] navigasi ke iloveimg.com/upscale-image ...');
